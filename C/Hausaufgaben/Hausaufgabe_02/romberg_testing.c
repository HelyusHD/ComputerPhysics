...
/***********************************************************************
 * Bsp. Integrand fuer Hausaufgaben 
 * hier moechte ich rumprobieren was ich mit romberg machen kann
 * 
 * alpha * q/a * exp ( -x^2/a^2 )
 ***********************************************************************/
double Ladungsdichte (double x, void * p)
{
  double const a = ((double*)p)[0]; // a wird auf p[0] gespeichert etc
  double const q = ((double*)p)[1];
  double const faktor = ((double*)p)[2];
  double const alpha = ((double*)p)[3];
  return( alpha * q/a * exp (-pow(x,2)/pow(a,2)));
}
/***********************************************************************
 * Hauptprogramm
 ***********************************************************************/
int main(int argc, char **argv) 
{
   ...
   * p Gamma ( p ) = Gamma ( p + 1 ) au snutzen
   *
   */ 
  func_type f = Ladungsdichte;

  double p[] = {4.0 , 1.0, 1.0, 1.0};
/*
  double p[0] = {4.0};  // hier initialiesiere ich die Parameter  
